...
def attention_validation_based(
    ...
    # 使用注意力权重聚合参数（a_i * n_i 加权）
    keys = updates[0][0].keys()
    out = {}
    total_weight = sum(w * n for (_, n), w in zip(updates, attention_weights))
    for k in keys:
        out[k] = sum(sd[k] * w * n for (sd, n), w in zip(updates, attention_weights)) / total_weight
...
def attention_validation_with_stability(
    ...
    if top_k is not None and top_k < len(attention_weights):
        # 找到top-k的索引
        topk_indices = sorted(range(len(final_scores)), key=lambda i: final_scores[i], reverse=True)[:top_k]
        # 其他客户端权重设为0
        for i in range(len(attention_weights)):
            if i not in topk_indices:
                attention_weights[i] = 0.0
    ...
    # 步骤5: 使用注意力权重聚合参数（a_i * n_i 加权）
    # 关键修复：使用 attention_weight * num_samples 作为最终权重
    # 这样既保留了FedAvg的样本数加权，又加入了质量筛选
    keys = updates[0][0].keys()
    out = {}
    total_weight = sum(w * n for (_, n), w in zip(updates, attention_weights))
    for k in keys:
...
def run_fedavg_with_attention(
    ...
    if attention_method == "stability":
        ...
        if top_k is not None:
            print(f"  → Top-k防塌缩: 每轮保留最好的{top_k}个客户端")
        ...
        if attention_method != "none" and attn_weights is not None:
            ...
            if attention_method == "stability" and perf_scores and stab_scores:
                ...
                print(f"[Round {rnd:03d}] AvgClientLoss {avg_tr_loss:.6f} AvgClientAcc {format_pct(avg_tr_acc)} | "
                      ...
                      f"Perf={avg_perf:.3f} Stab={avg_stab:.3f}")
            else:
                print(f"[Round {rnd:03d}] AvgClientAcc {format_pct(avg_tr_acc)} | "
                      f"TestAcc {format_pct(te_acc)} | Best {format_pct(best_acc)}@{best_round} | "
                      f"AttnW: max={max_weight:.3f} min={min_weight:.3f} std={std_weight:.3f}")
        else:
            print(f"[Round {rnd:03d}] AvgClientAcc {format_pct(avg_tr_acc)} | "
                  f"TestAcc {format_pct(te_acc)} | Best {format_pct(best_acc)}@{best_round}")
...
def main():
    ...
    if args.attention_method != "none":
        ...
        if args.top_k is not None:
            print(f"Top-k防塌缩: 每轮保留最好的{args.top_k}个客户端")
    print("="*80)
